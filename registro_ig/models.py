...
def select_all():
    """
    Devolvera una lista con todos los registros del fichero
     MOVIMIENTOS:FILE
    """
...
def select_by(id):
    """
    Devolver un registro con el id de la entrada o vacío si no lo encuentra
    MOVIMIENTOS_FILE
    """
    fichero = open(MOVIMIENTOS_FILE, "r", newline="")
    csvReader = csv.reader(fichero, delimiter=',', quotechar='"')
    registro_definitivo = None
...
def delete_by(id):
    """
    Borrar el registro cuyo id coincide con el de la entrada
    MOVIMIENTOS_FILE
    """
...
def insert(registro):
    ...
    Asignará al registro un id unico (acumulativo)
    MOVIMIENTOS_FILE
    """
    id = createId()
